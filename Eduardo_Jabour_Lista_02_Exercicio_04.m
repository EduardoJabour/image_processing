...
V1 = dct2(I1); ABSV1 = abs(V1);
V2 = dct2(I2); ABSV2 = abs(V2);
V3 = dct2(I3); ABSV3 = abs(V3);
V4 = dct2(I4); ABSV4 = abs(V4);
V5 = dct2(I5); ABSV5 = abs(V5);
V6 = dct2(I6); ABSV6 = abs(V6);
V7 = dct2(I7); ABSV7 = abs(V7);
V8 = dct2(I8); ABSV8 = abs(V8);
...
MAX = max(ABSV(:)); MEAN = mean(ABSV(:)); parametroA = ABSV(N*0.9,N*0.9); parametroB = ABSV(N*0.5,N*0.5); parametroC = ABSV(N*0.1,N*0.1);
...
RTVa = idct2(TVa); RTVb = idct2(TVb); RTVc = idct2(TVc);

str1 = sprintf('Reconstrucao %.4f', parametroA);
str2 = sprintf('Reconstrucao %.4f', parametroB);
str3 = sprintf('Reconstrucao %.4f', parametroC);

figure(3);
subplot(1, 4, 1); imshow(TI); title('Imagem fonte');
subplot(1, 4, 2); imshow(RTVa,[]); title(str1);
subplot(1, 4, 3); imshow(RTVb,[]); title(str2);
subplot(1, 4, 4); imshow(RTVc,[]); title(str3);
